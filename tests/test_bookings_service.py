...
def create_admin_token():
    ...
    r = c.post('/users/register', json={'username': 'admin', 'email': 'admin@example.com', 'password': 'AdminPass123', 'role': 'admin'})
    assert r.status_code == 201
    tok = c.post('/auth/login', json={'username': 'admin', 'password': 'AdminPass123'}).get_json()['access_token']
    return tok
...
def create_room(admin_tok, name='Room1', capacity=5):
    ...
    r = rc.post('/rooms', json={'name': name, 'capacity': capacity}, headers=_auth(admin_tok))
...
def test_admin_creates_booking_for_other_user(client):
    ...
    start, end = iso()
    r = client.post('/bookings', json={'user_id': bob_id, 'room_id': room_id, 'start_time': start, 'end_time': end}, headers=_auth(admin_tok))
...
def test_user_creates_own_booking(client):
    ...
    start, end = iso()
    r = client.post('/bookings', json={'user_id': alice_id, 'room_id': room_id, 'start_time': start, 'end_time': end}, headers=_auth(user_tok))
    assert r.status_code == 201
...
def test_list_bookings_admin_vs_user(client):
    ...
    assert client.post('/bookings', json={'user_id': u1_id,'room_id': room_id,'start_time': s1,'end_time': e1}, headers=_auth(user_tok1)).status_code == 201
    assert client.post('/bookings', json={'user_id': u2_id,'room_id': room_id,'start_time': s2,'end_time': e2}, headers=_auth(user_tok2)).status_code == 201
    # admin sees both
    admin_list = client.get('/bookings', headers=_auth(admin_tok)).get_json()
    assert len(admin_list) == 2
    # u1 sees only own
    u1_list = client.get('/bookings', headers=_auth(user_tok1)).get_json()
...
def test_user_updates_own_booking(client):
    ...
    s,e = iso(); b = client.post('/bookings', json={'user_id': sam_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json()
    new_end = (datetime.fromisoformat(e) + timedelta(minutes=30)).strftime('%Y-%m-%dT%H:%M:%S')
    r = client.patch(f"/bookings/{b['id']}", json={'end_time': new_end}, headers=_auth(user_tok))
...
def test_admin_updates_other_users_booking(client):
    ...
    s,e = iso(); b = client.post('/bookings', json={'user_id': tom_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json()
    new_start,new_end = iso(180,60)
    r = client.patch(f"/bookings/{b['id']}", json={'start_time': new_start,'end_time': new_end}, headers=_auth(admin_tok))
...
def test_admin_force_updates_non_active_booking(client):
    ...
    s,e = iso(); b = client.post('/bookings', json={'user_id': eve_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json()
    # cancel booking (active -> canceled)
    assert client.delete(f"/bookings/{b['id']}", headers=_auth(user_tok)).status_code == 200
    new_start,new_end = iso(240,60)
    r = client.patch(f"/bookings/{b['id']}", json={'start_time': new_start,'end_time': new_end,'force': True}, headers=_auth(admin_tok))
...
def test_admin_force_bypasses_conflict(client):
    ...
    b1 = client.post('/bookings', json={'user_id': zoe_id,'room_id': room_id,'start_time': s1,'end_time': e1}, headers=_auth(user_tok)).get_json()
    b2 = client.post('/bookings', json={'user_id': zoe_id,'room_id': room_id,'start_time': s2,'end_time': e2}, headers=_auth(user_tok)).get_json()
    # move second into overlap with first forcibly
    overlap_start, overlap_end = iso(60,60)  # inside first booking timeframe
    r = client.patch(f"/bookings/{b2['id']}", json={'start_time': overlap_start,'end_time': overlap_end,'force': True}, headers=_auth(admin_tok))
...
def test_user_cancels_own_booking(client):
    ...
    s,e = iso(); b = client.post('/bookings', json={'user_id': lee_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json()
    r = client.delete(f"/bookings/{b['id']}", headers=_auth(user_tok))
...
def test_admin_cancels_other_users_active_booking(client):
    ...
    s,e = iso(); b = client.post('/bookings', json={'user_id': mia_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json()
    r = client.delete(f"/bookings/{b['id']}", headers=_auth(admin_tok))
    assert r.status_code == 200
...
def test_admin_force_cancel_non_active_booking(client):
    ...
    s,e = iso(); b = client.post('/bookings', json={'user_id': neo_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json()
    assert client.delete(f"/bookings/{b['id']}", headers=_auth(user_tok)).status_code == 200
    r = client.post(f"/bookings/{b['id']}/force-cancel", headers=_auth(admin_tok))
    assert r.status_code == 200 and r.get_json()['status'] == 'canceled'
...
def test_check_availability_flow(client):
    ...
    s,e = iso(); r1 = client.get(f"/bookings/check?room_id={room_id}&start={s}&end={e}")
    assert r1.status_code == 200 and r1.get_json()['available'] is True
    assert client.post('/bookings', json={'user_id': ava_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).status_code == 201
    r2 = client.get(f"/bookings/check?room_id={room_id}&start={s}&end={e}")
...
def test_create_booking_forbidden_other_user(client):
    ...
    r = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok))
...
def test_create_booking_missing_fields(client):
    ...
    r = client.post('/bookings', json={'user_id': admin_id}, headers=_auth(admin_tok))
...
def test_create_booking_invalid_times(client):
    ...
    r = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': '2025-11-25T12:00:00','end_time': '2025-11-25T11:00:00'}, headers=_auth(admin_tok))
    assert r.status_code == 400

def test_create_booking_bad_iso(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='admin'"); admin_id = cur.fetchone()[0]; cur.close(); conn.close()
    r = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': 'bad','end_time': 'also'}, headers=_auth(admin_tok))
    assert r.status_code == 400

def test_create_booking_user_not_found(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); s,e = iso()
    r = client.post('/bookings', json={'user_id': 9999,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(admin_tok))
...
def test_create_booking_room_not_found(client):
    ...
    r = client.post('/bookings', json={'user_id': admin_id,'room_id': 9999,'start_time': s,'end_time': e}, headers=_auth(admin_tok))
    assert r.status_code == 404
...
def test_create_booking_conflict(client):
    ...
    assert client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(admin_tok)).status_code == 201
    r2 = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(admin_tok))
...
def test_create_booking_unauthorized_no_token(client):
    ...
    r = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s,'end_time': e})
    assert r.status_code == 401

def test_create_booking_invalid_token(client):
    r = client.post('/bookings', json={'user_id': 1,'room_id': 1,'start_time': 'x','end_time': 'y'}, headers={'Authorization': 'Bearer badtoken'})
    assert r.status_code == 401
...
def test_list_bookings_invalid_token(client):
    r = client.get('/bookings', headers={'Authorization': 'Bearer badtoken'})
    assert r.status_code == 401

def test_update_booking_not_found(client):
    admin_tok = create_admin_token()
    r = client.patch('/bookings/9999', json={'end_time': '2025-11-25T12:00:00'}, headers=_auth(admin_tok))
    assert r.status_code == 404

def test_update_booking_no_fields(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='admin'"); admin_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(admin_tok)).get_json()
    r = client.patch(f"/bookings/{b['id']}", json={}, headers=_auth(admin_tok))
    assert r.status_code == 400

def test_update_booking_invalid_times(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='admin'"); admin_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(admin_tok)).get_json()
    r = client.patch(f"/bookings/{b['id']}", json={'start_time': 'bad'}, headers=_auth(admin_tok))
    assert r.status_code == 400

def test_update_booking_conflict_without_force(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='admin'"); admin_id = cur.fetchone()[0]; cur.close(); conn.close()
    s1,e1 = iso(0,120); s2,e2 = iso(180,60); b1 = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s1,'end_time': e1}, headers=_auth(admin_tok)).get_json(); b2 = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s2,'end_time': e2}, headers=_auth(admin_tok)).get_json()
    overlap_s, overlap_e = iso(60,60)
    r = client.patch(f"/bookings/{b2['id']}", json={'start_time': overlap_s,'end_time': overlap_e}, headers=_auth(admin_tok))
    assert r.status_code == 409

def test_update_booking_room_not_found(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='admin'"); admin_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': admin_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(admin_tok)).get_json()
    r = client.patch(f"/bookings/{b['id']}", json={'room_id': 99999}, headers=_auth(admin_tok))
    assert r.status_code == 404
...
def test_update_booking_forbidden_other_user(client):
    ...
    conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='bob'"); bob_id = cur.fetchone()[0]; cur.execute("SELECT id FROM users WHERE username='carol'"); carol_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': bob_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json()
    r = client.patch(f"/bookings/{b['id']}", json={'end_time': e}, headers=_auth(other_tok))
    assert r.status_code == 403

def test_update_booking_non_active_without_force(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); user_tok = create_user_and_token('dan'); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='dan'"); dan_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': dan_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(user_tok)).get_json(); assert client.delete(f"/bookings/{b['id']}", headers=_auth(user_tok)).status_code == 200
    r = client.patch(f"/bookings/{b['id']}", json={'end_time': e}, headers=_auth(user_tok))
    assert r.status_code == 400

def test_update_booking_unauthorized_no_token(client):
    r = client.patch('/bookings/1', json={'end_time': '2025-11-25T11:00:00'})
    assert r.status_code == 401

def test_update_booking_invalid_token(client):
    r = client.patch('/bookings/1', json={'end_time': '2025-11-25T11:00:00'}, headers={'Authorization': 'Bearer bad'})
    assert r.status_code == 401

def test_cancel_booking_not_found(client):
    admin_tok = create_admin_token(); r = client.delete('/bookings/9999', headers=_auth(admin_tok))
    assert r.status_code == 404

def test_cancel_booking_forbidden_other_user(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); tok1 = create_user_and_token('aa'); tok2 = create_user_and_token('bb'); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='aa'"); aa_id = cur.fetchone()[0]; cur.execute("SELECT id FROM users WHERE username='bb'"); bb_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': aa_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(tok1)).get_json()
    r = client.delete(f"/bookings/{b['id']}", headers=_auth(tok2))
    assert r.status_code == 403

def test_cancel_booking_non_active(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); tok = create_user_and_token('cc'); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='cc'"); cc_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': cc_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(tok)).get_json(); assert client.delete(f"/bookings/{b['id']}", headers=_auth(tok)).status_code == 200
    r = client.delete(f"/bookings/{b['id']}", headers=_auth(tok))
    assert r.status_code == 400
    r_admin = client.delete(f"/bookings/{b['id']}", headers=_auth(admin_tok))
    assert r_admin.status_code == 400

def test_cancel_booking_unauthorized_no_token(client):
    r = client.delete('/bookings/1')
    assert r.status_code == 401

def test_cancel_booking_invalid_token(client):
    r = client.delete('/bookings/1', headers={'Authorization': 'Bearer bad'})
    assert r.status_code == 401

def test_force_cancel_not_found(client):
    admin_tok = create_admin_token(); r = client.post('/bookings/99999/force-cancel', headers=_auth(admin_tok))
    assert r.status_code == 404

def test_force_cancel_forbidden_non_admin(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok); tok = create_user_and_token('dd'); conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT id FROM users WHERE username='dd'"); dd_id = cur.fetchone()[0]; cur.close(); conn.close(); s,e = iso(); b = client.post('/bookings', json={'user_id': dd_id,'room_id': room_id,'start_time': s,'end_time': e}, headers=_auth(tok)).get_json()
    r = client.post(f"/bookings/{b['id']}/force-cancel", headers=_auth(tok))
    assert r.status_code == 403

def test_force_cancel_invalid_token(client):
    r = client.post('/bookings/1/force-cancel', headers={'Authorization': 'Bearer bad'})
    assert r.status_code == 401

def test_force_cancel_unauthorized_no_token(client):
    r = client.post('/bookings/1/force-cancel')
    assert r.status_code == 401

def test_check_availability_missing_params(client):
    r = client.get('/bookings/check?room_id=1&start=2025-11-25T10:00:00')
    assert r.status_code == 400

def test_check_availability_bad_room_id(client):
    s,e = iso(); r = client.get(f"/bookings/check?room_id=abc&start={s}&end={e}")
    assert r.status_code == 400

def test_check_availability_invalid_times(client):
    admin_tok = create_admin_token(); room_id = create_room(admin_tok)
    r = client.get(f"/bookings/check?room_id={room_id}&start=2025-11-25T12:00:00&end=2025-11-25T11:00:00")
    assert r.status_code == 400

def test_check_availability_room_not_found(client):
    s,e = iso(); r = client.get(f"/bookings/check?room_id=99999&start={s}&end={e}")
    assert r.status_code == 404
